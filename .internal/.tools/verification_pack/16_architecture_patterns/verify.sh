#!/usr/bin/env bash
# Drills déterministes pour 16_architecture_patterns. Chaque drill compare une sortie
...
if [ "$fail" -gt 0 ]; then
  echo "== [16_architecture_patterns] KO ($fail échec(s)) =="
  exit 1
fi
echo "== [16_architecture_patterns] OK =="
