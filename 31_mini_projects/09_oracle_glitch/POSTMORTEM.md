...
Les premiers tests de `edgeCases.test.js` vérifiaient surtout des cas de malformation JSON (`truncated`, `missing fields`). Les vrais edge cases JS (`NaN === NaN`, `0.1 + 0.2`, `undefined` dans un tableau) sont arrivés en deuxième passe, après une relecture du module `29_edge_cases`.
