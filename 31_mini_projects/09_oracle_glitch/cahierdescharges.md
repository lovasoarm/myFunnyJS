...
### `30_oop_js` : classes, prototype chain, mixins
...
### `28_team_craft` : code review outillée, ADR
...
### `29_edge_cases` : NaN, floating point, undefined dans des tableaux
...
30_oop_js        --> CodeAnalyzer, Validator -> StrictValidator -> LLMOutputValidator, mixins
28_team_craft    --> src/review/ (review automatisée), ADR/ (toutes les décisions du pipeline)
29_edge_cases    --> src/edgeCases/ (injecteur de pièges), tests/edgeCases.test.js
```
...
- Composition (passer des validators en paramètre) : valide aussi. Choix de la
  ...
  l'objectif pédagogique du module 30_oop_js).
