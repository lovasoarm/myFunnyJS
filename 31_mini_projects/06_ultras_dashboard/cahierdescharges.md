...
### `27_observability` : logging, tracing, métriques, Sentry
...
### `26_scalability` : rate limiting, message queues, load balancing simulé
...
```
27_observability  --> src/observability/ (logger, tracer, metrics, sentry)
26_scalability    --> src/queue/ (message queue), src/balancer/ (round-robin simulé)
