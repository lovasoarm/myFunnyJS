...
Pas dans les "6 blocs prioritaires", mais c'est un module de soutien que tu vas croiser partout ensuite. `07_data_structures` : les hash tables reposent directement sur le hashing. `08_algorithms` : les probabilités et la combinatoire interviennent dans plusieurs patterns. `01_rasengan_engine` : l'arithmétique modulaire gère les cooldowns de chakra et les critiques, les probabilités calculent les esquives et les ratés. Sans ce module, ces mini-projets te demanderont d'inventer des roues déjà existantes.
