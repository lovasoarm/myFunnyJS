...
Les **seuls** falsy (mémorise-les, ils sont 6) :

```
false, 0, "", null, undefined, NaN
```

Tout le reste est truthy. Y compris des trucs surprenants :
...
Oui. Même un tableau vide est `true`. Pourquoi ? Parce qu'il **existe**. Et pour JS, exister = vrai. Philosophique.
...
- Utiliser `===`
- Tester `NaN` correctement avec `Number.isNaN()`
- Ne jamais croire un input

Les types sont neutres. C'est ton ignorance qui crée les bugs.

Si tu maîtrises ça, tu écris du code propre. Sinon tu écris du code fragile.
