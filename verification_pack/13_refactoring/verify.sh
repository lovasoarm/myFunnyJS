#!/usr/bin/env bash
# Drill de vérification déterministe pour 13_refactoring
set -e
...
if ls "$ROOT/../13_refactoring"/*grimoire*.md >/dev/null 2>&1 || \
   find "$ROOT/../13_refactoring" -name '*grimoire*.md' | grep -q .; then
...
if [ -f "$ROOT/../13_refactoring/00_why_"*".md" ] || ls "$ROOT/../13_refactoring/00_why_"*.md >/dev/null 2>&1; then
