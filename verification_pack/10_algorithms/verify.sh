#!/usr/bin/env bash
# Drill de vérification déterministe pour 10_algorithms
set -e
...
if ls "$ROOT/../10_algorithms"/*grimoire*.md >/dev/null 2>&1 || \
   find "$ROOT/../10_algorithms" -name '*grimoire*.md' | grep -q .; then
...
if [ -f "$ROOT/../10_algorithms/00_why_"*".md" ] || ls "$ROOT/../10_algorithms/00_why_"*.md >/dev/null 2>&1; then
