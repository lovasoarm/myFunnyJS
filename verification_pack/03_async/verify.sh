#!/usr/bin/env bash
# Drill de vérification déterministe pour 03_async
set -e
...
if ls "$ROOT/../03_async"/*grimoire*.md >/dev/null 2>&1 || \
   find "$ROOT/../03_async" -name '*grimoire*.md' | grep -q .; then
...
if [ -f "$ROOT/../03_async/00_why_"*".md" ] || ls "$ROOT/../03_async/00_why_"*.md >/dev/null 2>&1; then
