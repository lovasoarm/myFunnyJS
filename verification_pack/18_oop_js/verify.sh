#!/usr/bin/env bash
# Drill de vérification déterministe pour 18_oop_js
set -e
...
if ls "$ROOT/../18_oop_js"/*grimoire*.md >/dev/null 2>&1 || \
   find "$ROOT/../18_oop_js" -name '*grimoire*.md' | grep -q .; then
...
if [ -f "$ROOT/../18_oop_js/00_why_"*".md" ] || ls "$ROOT/../18_oop_js/00_why_"*.md >/dev/null 2>&1; then
