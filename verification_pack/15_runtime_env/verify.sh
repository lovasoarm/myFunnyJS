#!/usr/bin/env bash
# Drill de vérification déterministe pour 15_runtime_env
set -e
...
if ls "$ROOT/../15_runtime_env"/*grimoire*.md >/dev/null 2>&1 || \
   find "$ROOT/../15_runtime_env" -name '*grimoire*.md' | grep -q .; then
...
if [ -f "$ROOT/../15_runtime_env/00_why_"*".md" ] || ls "$ROOT/../15_runtime_env/00_why_"*.md >/dev/null 2>&1; then
