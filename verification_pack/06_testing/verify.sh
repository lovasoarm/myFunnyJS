#!/usr/bin/env bash
# Drill de vérification déterministe pour 06_testing
set -e
...
if ls "$ROOT/../06_testing"/*grimoire*.md >/dev/null 2>&1 || \
   find "$ROOT/../06_testing" -name '*grimoire*.md' | grep -q .; then
...
if [ -f "$ROOT/../06_testing/00_why_"*".md" ] || ls "$ROOT/../06_testing/00_why_"*.md >/dev/null 2>&1; then
