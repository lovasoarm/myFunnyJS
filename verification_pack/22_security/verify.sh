#!/usr/bin/env bash
# Drill de vérification déterministe pour 22_security
set -e
...
if ls "$ROOT/../22_security"/*grimoire*.md >/dev/null 2>&1 || \
   find "$ROOT/../22_security" -name '*grimoire*.md' | grep -q .; then
...
if [ -f "$ROOT/../22_security/00_why_"*".md" ] || ls "$ROOT/../22_security/00_why_"*.md >/dev/null 2>&1; then
