#!/usr/bin/env bash
# Drill de vérification déterministe pour 12_design_patterns
set -e
...
if ls "$ROOT/../12_design_patterns"/*grimoire*.md >/dev/null 2>&1 || \
   find "$ROOT/../12_design_patterns" -name '*grimoire*.md' | grep -q .; then
...
if [ -f "$ROOT/../12_design_patterns/00_why_"*".md" ] || ls "$ROOT/../12_design_patterns/00_why_"*.md >/dev/null 2>&1; then
