#!/usr/bin/env bash
# Drill de vérification déterministe pour 05_error_handling
set -e
...
if ls "$ROOT/../05_error_handling"/*grimoire*.md >/dev/null 2>&1 || \
   find "$ROOT/../05_error_handling" -name '*grimoire*.md' | grep -q .; then
...
if [ -f "$ROOT/../05_error_handling/00_why_"*".md" ] || ls "$ROOT/../05_error_handling/00_why_"*.md >/dev/null 2>&1; then
