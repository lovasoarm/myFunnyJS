...
Module : **23_ai_native_dev** : developpement AI-native (prompts, agents, contexte).
...
- **Python** : Python : ecosysteme dominant (`langchain`, `llamaindex`, `openai`) ; memes patterns (context window, tool use, guardrails).
- **Go** : Go : ecosysteme LLM plus jeune (`langchaingo`) ; memes primitives (prompt, tool, contexte) ; concurrence native pour orchestrer les appels.
- **Rust** : Rust : ecosysteme LLM emergent (`rig`, `llm-chain`) ; utile pour l'inference locale (`llama.cpp` bindings) plus que pour l'orchestration.
