...
| **Long method** | Fonction trop longue qui fait plusieurs choses sans le dire. | `function processAll(data) {` ` // parse` ` // valide` ` // transforme` ` // log` `// 4 jobs en 1 fonction` `}` | Prison Break (plan trop long, trop de variables) / Attack on Titan (mission sans étapes) |
...
| **Safety net (filet de tests)** | Tests qui figent le comportement actuel avant de toucher au code. | `test('getTotal sur 2 tracks = 380', () => {` ` expect(pm.getTotal()).toBe(380)` `// si ça casse après refacto, ce test devient rouge` `})` | Prison Break (vérifier le plan avant de creuser) / Attack on Titan (équipement de sécurité avant la sortie) |
