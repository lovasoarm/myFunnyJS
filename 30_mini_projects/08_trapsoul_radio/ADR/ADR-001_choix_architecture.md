...
Le projet couvre `14_typescript`, `19_i18n`, `18_accessibility`. Le système de traduction est la colonne vertébrale de l'interface : une clé manquante brise silencieusement l'expérience de tous les auditeurs de cette locale.
...
- Rejeté parce que : le projet enseigne l'API native `Intl` (DateTimeFormat, NumberFormat, PluralRules) sans bibliothèque externe : utiliser i18next contourne l'apprentissage central du module `19_i18n` ; et le typage natif TS sur les clés de traduction est précisément ce qu'on veut démontrer
