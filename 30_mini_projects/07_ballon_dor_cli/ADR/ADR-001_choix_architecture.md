...
Le projet est un outil CLI Node.js. Il couvre `15_runtime_env` (process.argv, filesystem, Worker Threads) et `04_error_handling`. Ce n'est pas une API avec des connexions persistantes : chaque commande est un processus Node qui démarre, lit/écrit, et se termine.
