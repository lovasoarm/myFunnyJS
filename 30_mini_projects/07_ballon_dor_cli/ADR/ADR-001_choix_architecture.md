...
Le projet est un outil CLI Node.js. Il couvre `16_runtime_env` (process.argv, filesystem, Worker Threads) et `05_error_handling`. Ce n'est pas une API avec des connexions persistantes : chaque commande est un processus Node qui démarre, lit/écrit, et se termine.
...
- Rejeté parce que : ce projet enseigne le filesystem Node.js (`fs`, `path`, streams) : utiliser SQLite contourne l'apprentissage central du module `16_runtime_env`
...
- `fs.readFileSync` et `fs.writeFileSync` sont les APIs centrales de `16_runtime_env` : leur usage sur de vraies données les ancre dans un contexte concret

Sacrifices :
- les écritures concurrentes de plusieurs processus Node simultanés (ex : deux terminaux qui votent en même temps) ne sont pas protégées par un lock : un vote peut écraser l'autre ; acceptable pour un outil mono-shinobi, problématique en prod multi-process
