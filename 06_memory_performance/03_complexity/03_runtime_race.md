...
const LARGE = 100_000
const sortedData = generateData(LARGE)
  .sort((a, b) => a.id - b.id)
...
for (const n of sizes2) {
  ...
  console.log(`\n── n = ${n} ──`)
  if (n <= 10_000) {  // au-delà bubble sort prend trop de temps pour être intéressant
...
*(Indice : si l'algo est vraiment O(n²) vs O(n log n), le ratio devrait être proportionnel à `n / log n`)*
...
*(Indice : le problème "two sum" a une solution O(n) avec une Map)*
