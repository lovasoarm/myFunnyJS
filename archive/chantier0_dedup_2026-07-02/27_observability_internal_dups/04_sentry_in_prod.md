...
Sentry.init({
  ...
  tracesSampleRate: 0.1 // sampling, vu dans `27_observability/02_distributed_tracing`
})
...
Le pourquoi c'est puissant : la même `TypeError` capturée avec contexte devient "cette erreur arrive systématiquement pour les titans sans `shippingAddress`, sur la fonctionnalité checkout, depuis le déploiement de 14h32". Sans contexte, c'est juste une ligne de stack trace anonyme parmi des centaines d'autres.
