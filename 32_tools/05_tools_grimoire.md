...
| -------------------------- | ---------------------------------------------------------------------------------------------------------- | ------------------------------------------------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ------- |
| Logger structuré           | Outil qui enregistre des événements avec horodatage, niveau de gravité, et contexte, en format exploitable | `logger.error("echec", { secteur: "nord" })`      | le Sharingan de Kakashi : tout ce qui s'est passé, dans l'ordre, avec les détails / le TDD_JOURNAL de Michael Scofield : chaque étape documentée, pas juste le résultat final | L'image simplifie : le mécanisme runtime réel peut différer (spécificités moteur JS, edge cases, timing). |
| Niveau de log              | Hiérarchie de gravité (debug, info, warn, error) qui permet de filtrer le bruit selon le contexte          | `creerLogger("warn")` ignore debug et info        | la jauge de chakra dans Naruto : debug=réserve totale, error=niveau critique / les niveaux de menace du camp de Rick Grimes : vert, orange, rouge                             | L'image simplifie : le mécanisme runtime réel peut différer (spécificités moteur JS, edge cases, timing). |
| Contexte (log)             | Données additionnelles jointes à un log, qui transforment un message vague en indice complet               | `{ frequence: 145.5, tentative: 3 }`              | le briefing complet que Michael donne à Lincoln, pas juste "problème au couloir" / le rapport de mission de Garo : secteur, Horror, résultat, état de l'armure                | L'image simplifie : le mécanisme runtime réel peut différer (spécificités moteur JS, edge cases, timing). |
| Benchmark                  | Mesure comparative de performance entre deux implémentations, sur plusieurs itérations                     | `comparer([{nom, fn}, {nom, fn}])`                | le match de vitesse Naruto vs Sasuke chronométré par Kakashi / le Ballon d'Or : les stats réelles sur toute la saison, pas l'impression du foot du dimanche                   | L'image simplifie : le mécanisme runtime réel peut différer (spécificités moteur JS, edge cases, timing). |
| Warmup                     | Exécutions préliminaires avant la vraie mesure, pour laisser le moteur JS s'optimiser (JIT)                | boucle de 10 appels avant le vrai chronométrage   | l'entraînement de Goku en Chambre du Temps avant le vrai combat / Vegeta qui ne mesure sa puissance qu'après avoir atteint le pic, pas au démarrage à froid                   | L'image simplifie : le mécanisme runtime réel peut différer (spécificités moteur JS, edge cases, timing). |
| performance.now()          | Fonction native qui retourne un timestamp précis en millisecondes, pour mesurer des durées                 | `const debut = performance.now()`                 | le chronomètre du tournoi Chunin, pas l'horloge du village / le compte à rebours de l'armure Garo : 99.9 secondes, précis à la milliseconde                                   | L'image simplifie : le mécanisme runtime réel peut différer (spécificités moteur JS, edge cases, timing). |
| Facteur comparatif         | Rapport entre deux mesures de benchmark, plus lisible qu'une différence brute en millisecondes             | `dureeMoyenne_ms / plusRapide.dureeMoyenne_ms`    | "Naruto est 3x plus rapide que Sasuke sur ce jutsu" vs "Naruto prend 0.002ms de moins" / le classement Ballon d'Or en ratio, pas en points bruts                              | L'image simplifie : le mécanisme runtime réel peut différer (spécificités moteur JS, edge cases, timing). |
| Assertion                  | Vérifie qu'une hypothèse sur l'état du code est vraie, plante immédiatement sinon avec le contexte exact   | `assert(stock >= 0, "stock invalide", { stock })` | le détecteur de Horror dans Garo : si le signal est là, l'armure s'active immédiatement / Rick Grimes qui vérifie chaque pièce avant d'avancer, jamais à l'aveugle            | L'image simplifie : le mécanisme runtime réel peut différer (spécificités moteur JS, edge cases, timing). |
| Inspecteur d'état          | Affiche un snapshot figé et complet d'une donnée à un instant précis, sans casser le flux du code          | `inspecter("avant rotation", etat)`               | l'arrêt sur image dans Walking Dead avant une décision critique / le screenshot de l'état du camp à T=0, pas une description de mémoire 3 heures plus tard                    | L'image simplifie : le mécanisme runtime réel peut différer (spécificités moteur JS, edge cases, timing). |
| structuredClone            | Fonction native qui fait une copie profonde d'un objet, indépendante de l'original                         | `const snapshot = structuredClone(donnees)`       | le clone de Naruto qui garde une copie exacte du moment où il a été créé / le plan tatoué sur Michael, figé à l'instant de l'impression                                       | L'image simplifie : le mécanisme runtime réel peut différer (spécificités moteur JS, edge cases, timing). |
| Traceur                    | Enregistre une suite d'étapes d'exécution avec timestamp et données, pour reconstituer un chemin complet   | `creerTraceur()` puis `tracer(nom, donnees)`      | le replay que Kakashi fait mentalement après un combat pour comprendre ce qui s'est passé / le journal de vol de l'armure Garo : chaque étape du combat, dans l'ordre         | L'image simplifie : le mécanisme runtime réel peut différer (spécificités moteur JS, edge cases, timing). |
