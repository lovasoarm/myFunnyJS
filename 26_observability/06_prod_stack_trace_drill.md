...
La plupart des pipelines CI/CD uploadent la source map vers l'outil de tracking d'erreurs (Sentry, par exemple) au moment du build, sans jamais l'exposer publiquement. Le fichier `.map` existe quelque part, même s'il n'est pas accessible depuis le navigateur de l'opérateur final.
