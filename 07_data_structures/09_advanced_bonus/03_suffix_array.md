...
function buildSuffixArray(str) {
  ...
  for (let gap = 1; gap < n; gap *= 2) {
    ...
    for (let i = 1; i < n; i++) {
      ...
      rank[curr] = rank[prev] +
        (rankCopy[prev] !== rankCopy[curr] || prevSecond !== currSecond ? 1 : 0)
    }
...
function searchPattern(str, sa, pattern) {
  ...
  while (lo <= hi) {
    ...
    if (suffix === pattern)      { first = mid; hi = mid - 1 }  // continue à gauche
  ...
  while (lo <= hi) {
    ...
    if (suffix === pattern)    { last = mid; lo = mid + 1 }  // continue à droite
    else if (suffix < pattern) lo = mid + 1
    else                       hi = mid - 1
  }
...
```js
const lyrics = "aint nobody gonna love you like i love you baby nobody like nobody"
...
```js
const jutsu = "rasengan rasenshuriken shadow clone great ball rasengan chidori chidori blade"
```
...
const analyse2 = "la technique de balle et la vision du jeu de messi sont uniques"
```
