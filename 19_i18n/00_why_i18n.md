...
Ton app marche super bien, jusqu'à ce qu'un utilisateur japonais voit une date affichée au format américain et la lise complètement à l'envers. Jusqu'à ce qu'un utilisateur allemand voie un prix avec une virgule à la place d'un point et pense que ton produit coûte 1000 fois moins cher qu'en réalité. Jusqu'à ce qu'un utilisateur arabe découvre que ta pluralisation codée pour le français explose dès qu'il y a plus de deux résultats, parce que sa langue a six formes plurales et que ton code n'en gère qu'une.
...
```
texte affiché à l'utilisateur                          --> clé de traduction      --> jamais de texte en dur
...
L'i18n touche absolument tout ce qui s'affiche à l'utilisateur : pas juste les textes de bouton, mais chaque date, chaque nombre, chaque message qui dépend du contexte de quantité.
...
Pas dans les 6 blocs prioritaires explicitement listés, mais directement intégré dans le mini-projet `08_trapsoul_radio`, qui combine `14_typescript`, `17_web_concepts`, `18_accessibility`, et `19_i18n` pour une plateforme qui doit gérer plusieurs langues avec pluralisation et formats corrects par locale, en plus d'être accessible.
...
**Lien avec les autres modules :** `14_typescript` — les clés de traduction typées en TypeScript (si une clé n'existe pas en traduction, erreur de compilation). `24_databases` — les préférences de langue des utilisateurs sont souvent persistées en base.
