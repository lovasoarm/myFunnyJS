...
Inconvénient : complexité ajoutée, cohérence éventuelle (vu dans `25_databases/02_nosql_basics`) au lieu d'immédiate.
...
Le vrai intérêt : la dead letter queue devient un endroit que l'équipe surveille (vu dans `27_observability`), pour comprendre pourquoi certains messages échouent systématiquement, sans que ça bloque le traitement normal des autres messages.
