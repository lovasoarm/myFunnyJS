...
| **Complexité spatiale** | Quantité de mémoire supplémentaire utilisée par un algorithme, indépendamment du temps | `// O(n) spatiale : on crée un tableau de même taille` `const result = arr.map(x => x * 2)` | Le nombre de carnets de notes dont Walter a besoin pour calculer sa formule / le nombre de cellules qu'il faut réserver à Fox River pour faire évader tout le monde d'un coup |
