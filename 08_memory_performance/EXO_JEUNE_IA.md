...
---
# EXO [JEUNE IA] : 08_memory_performance
...
```bash
bash .internal/.tools/verification_pack/08_memory_performance/verify.sh solution.js
...
Comprendre a la main un Set de dedup, une fuite via closure ou un listener non retire, c'est ce qui te sauve d'une fuite en prod six mois plus tard. Si l'IA a ecrit ton code memoire, tu ne verras pas le leak arriver.
...
```bash
bash .internal/.tools/verification_pack/_jeune_ia/run_fasting_drill.sh 45 "drill memory_performance"
```
