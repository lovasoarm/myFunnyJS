...
Prérequis : `01_fundamentals` complet, `10_design_patterns` (notamment observer et factory).
...
class SurvivorView {
  ...
  constructor(containerId) {
    ...
    if (!this.#container) throw new Error(`Container #${containerId} introuvable dans le DOM`)
  }
...
class BadController {
  handleAdd(name, role) {
    if (!name) { alert("Nom requis") ; return }   // validation ici : non
    if (name.length < 2) { alert("Trop court") ; return }   // encore : non
...
Au lieu que le Controller appelle `view.render()` manuellement, le Model peut émettre des événements (Observer Pattern de `10_design_patterns`) :
