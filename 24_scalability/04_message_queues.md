...
Inconvénient : complexité ajoutée, cohérence éventuelle (vu dans `23_databases/02_nosql_basics`) au lieu d'immédiate.
...
Le pourquoi : l'utilisateur reçoit une réponse en quelques centaines de millisecondes au lieu d'attendre 45 secondes. Le traitement lourd continue en arrière-plan, et l'utilisateur peut être notifié plus tard (websocket vu dans `19_realtime`, email, ou juste un statut qui change quand il rafraîchit).
...
La correction : rendre l'opération idempotente (vue aussi dans `20_api_craft/04_auth_jwt` pour le refresh token), c'est-à-dire que la rejouer plusieurs fois donne le MÊME résultat que la jouer une fois.
...
Le vrai intérêt : la dead letter queue devient un endroit que l'équipe surveille (vu dans `25_observability`), pour comprendre pourquoi certains messages échouent systématiquement, sans que ça bloque le traitement normal des autres messages.
