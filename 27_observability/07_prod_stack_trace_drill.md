...
La plupart des pipelines CI/CD uploadent la source map vers l'outil de tracking d'erreurs (Sentry, par exemple) au moment du build, sans jamais l'exposer publiquement. Le fichier `.map` existe quelque part, même s'il n'est pas accessible depuis le navigateur de l'shinobi final.
...
En architecture microservices, l'erreur que tu vois côté frontend n'est parfois que le symptôme d'un crash survenu trois services plus loin. Une stack trace seule ne le montre pas : il faut la croiser avec le `traceId` (module `27_observability/02_distributed_tracing`).
