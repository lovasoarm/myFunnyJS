...
### `25_observability` : logging, tracing, métriques, Sentry
...
### `24_scalability` : rate limiting, message queues, load balancing simulé
...
### `13_typescript` : generics, utility types, types stricts sur tout le pipeline
...
```
25_observability  --> src/observability/ (logger, tracer, metrics, sentry)
24_scalability    --> src/queue/ (message queue), src/balancer/ (round-robin simulé)
13_typescript     --> generics Event<T>, Pipeline<I,O>, utility types sur les structs
```
