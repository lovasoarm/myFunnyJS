...
### `22_ai_native_dev` : workflow IA, prompt engineering, validation
...
### `28_oop_js` : classes, prototype chain, mixins
...
### `26_team_craft` : code review outillée, ADR
...
### `27_edge_cases` : NaN, floating point, undefined dans des tableaux
...
```
22_ai_native_dev --> src/prompt/ (PromptBuilder), src/validator/ (schema), src/streaming/
28_oop_js        --> CodeAnalyzer, Validator -> StrictValidator -> LLMOutputValidator, mixins
26_team_craft    --> src/review/ (review automatisée), ADR/ (toutes les décisions du pipeline)
27_edge_cases    --> src/edgeCases/ (injecteur de pièges), tests/edgeCases.test.js
```
...
- Composition (passer des validators en paramètre) : valide aussi. Choix de la
  ...
  l'objectif pédagogique du module 28_oop_js).
