...
### `04_testing` : unit, intégration, mocking, E2E Playwright
...
### `11_refactoring` : SOLID sur du code procédural, code smells
...
### `14_runtime_env` : CLI Node.js, fs, Worker Threads
...
### `31_tools` : logger structuré, benchmark, debug toolkit
...
```
04_testing    --> tests/ (unit + integration), e2e/ (Playwright), mocks/
11_refactoring --> legacy/ -> src/ (SOLID, code smells éliminés)
14_runtime_env --> src/cli.js (argv), src/store/fileStore.js (fs), src/workers/
31_tools       --> src/logger/ (JSON structuré), src/debug/ (replay de scénarios)
```
