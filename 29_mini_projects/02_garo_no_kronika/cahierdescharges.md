...
### `02_async` : Promises, async/await, race, allSettled
...
### `03_error_handling` : propagation et stratégies d'erreur
...
### `15_architecture_patterns` : event-driven, module pattern
...
```
02_async             --> dispatcher.js (allSettled), missionRunner.js (race + timeout)
03_error_handling    --> errors/ (custom errors), propagation dans missionRunner.js
19_realtime          --> streamEmitter.js (Chevalier émet), streamReceiver.js (Conseil écoute)
...
Le point de résistance majeur est `missionRunner.js`. La combinaison de `Promise.race`, d'un timeout, et de la propagation d'erreurs typées dans un même bloc async est précisément ce que le module `02_async` + `03_error_handling` préparent. Si tu bloques ici, relis `03_error_handling/03_error_propagation.md`.
