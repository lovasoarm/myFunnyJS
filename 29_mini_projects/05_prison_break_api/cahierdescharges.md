...
### `20_api_craft` : Express, CRUD, middleware, versioning
...
### `21_security` : JWT, bcrypt, XSS, injection, rate limiting
...
### `23_databases` : SQLite, modélisation, indexes, Redis cache
...
### `16_web_concepts` : HTTP, status codes, headers, caching
...
```
20_api_craft    --> src/routes/, src/middleware/errorHandler.js, src/server.js
21_security     --> src/auth/ (JWT + bcrypt), src/middleware/rateLimiter.js + sanitizer.js
23_databases    --> src/db/ (SQLite), src/cache/ (Redis simulé)
16_web_concepts --> status codes, headers, format d'erreur uniforme
```
...
| passwordService + jwtService | 2h | Faible si le module 21 est maîtrisé |
