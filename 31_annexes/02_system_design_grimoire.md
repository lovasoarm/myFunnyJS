...
| CSRF | Un site malveillant forge une requête vers ton app depuis le navigateur d'un shinobi connecté. Le navigateur envoie les cookies de session automatiquement : la requête arrive authentifiée sans consentement réel. | `app.use(csrf({ cookie: true })); res.render('mission', { csrfToken: req.csrfToken() });` | T-Bag qui signe un ordre de transfert à ta place pendant que t'as le dos tourné / un faux bouton "confirmer mission" dans un message : tu cliques, tu valides un truc que t'as jamais voulu |
