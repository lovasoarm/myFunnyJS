...
Sentry.init({
  ...
  tracesSampleRate: 0.1 // sampling, vu dans `25_observability/02_distributed_tracing`
})

// Capture manuelle d'une erreur attrapée volontairement (vu dans `03_error_handling`)
try {
...
Le pourquoi : Sentry ne remplace ni try/catch ni tes custom errors (vus dans `03_error_handling/02_custom_errors`), il les complète. Ton code continue de décider comment réagir à l'erreur (retry, fallback, fail-fast), Sentry capture en parallèle ce qui s'est passé pour que l'équipe puisse l'analyser après coup, sans avoir eu besoin de reproduire le bug à la main.
...
Pour une erreur qui survient pendant le processus de checkout (vu aussi dans `20_api_craft/02_rest_crud_complete`), liste les 5 informations de contexte les plus utiles à attacher avant la capture, et explique pour chacune ce qu'elle permettrait de diagnostiquer plus vite. (15 minutes)
