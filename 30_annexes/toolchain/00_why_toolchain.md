...
Docker        --> après avoir compris runtime Node (module 14_runtime_env)
CI/CD         --> après avoir des tests à automatiser (module 04_testing)
```
...
14_runtime_env        --> Node, process, filesystem : indispensable pour Docker et les scripts CI
04_testing            --> sans tests, CI/CD c'est juste un déploiement automatisé aveugle
```

Ce qui devient plus simple après ce module :
```
29_mini_projects       --> tu containerises et tu déploies tes projets sans bloquer dessus
toute collaboration future --> Git devient un réflexe, pas un obstacle
21_security            --> tu comprends déjà ce qu'est une image Docker, donc les surfaces d'attaque liées
```
