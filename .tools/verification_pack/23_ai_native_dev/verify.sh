#!/usr/bin/env bash
# Drills déterministes pour 23_ai_native_dev. Chaque drill compare une sortie réelle
# produite par Node à une sortie attendue exacte (byte-for-byte).
set -u
...
if [ "$fail" -gt 0 ]; then
  echo "== [23_ai_native_dev] KO ($fail échec(s)) =="
  exit 1
fi
echo "== [23_ai_native_dev] OK =="
