---
stability: intemporel
---

# 00 : Prereq check : Web Concepts
Temps de lecture ~5 min

> Tu ne dois **pas** entrer dans ce module si tu ne peux pas répondre à ces questions
> **sans regarder**. Ce n'est pas un test noté, c'est un filtre anti-illusion.
> Ces questions portent sur `16_architecture_patterns`, le module que tu viens de finir.

## Questions

1. Quel est le seul vrai but de l'architecture logicielle ?
2. Différence entre couplage et cohésion ?
3. MVC, en une phrase ?
4. Différence entre monolithe et microservices, en une phrase ?

## Verdict

- **3+ réponses solides** → tu peux entrer.
- **2 ou moins** → retour à `16_architecture_patterns/`, ou à sa synthèse `07_architecture_grimoire.md`.

> Se sentir "prêt" ≠ être prêt. Les questions ci-dessus tranchent.

> **Note pour ce module précis** : la différence CORS/CSP, ce qui bloque le
> rendu d'une page, et les directives Cache-Control sont le contenu que ce
> module va t'enseigner (notamment `01_http_rest_basics.md`) : normal de
> ne pas encore les maîtriser. Ta compréhension est testée en fin de
> module, dans `08_web_concepts_grimoire.md`.

---
stability: stable
