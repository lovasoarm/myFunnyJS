...
Cas concret : tu ne veux pas qu'un bug ou qu'un utilisateur mette des données invalides dans l'état.
    ...
    set(target, property, value) {
      ...
      if (property === "chakra") {
        if (typeof value !== "number") {
          throw new TypeError(
            `chakra doit être un nombre, reçu : ${typeof value}`,
          );
        }
        if (value < 0 || value > 100) {
          throw new RangeError(
            `chakra hors limites : ${value} (attendu 0-100)`,
          );
        }
      }
    ...
    {},
    {
      get(target, property) {
        ...
        if (property === "careerGoals") {
          console.log(
            `[COMPUTE] calcul des buts en carrière pour ${playerId}...`,
          );
...
function createProtectedFormula(data) {
  ...
  return new Proxy(privateData, {
    ...
    set(target, property, value) {
      if (typeof property === "string" && property.startsWith("_")) {
        throw new Error(
          `Modification refusée : ${property} est en lecture seule`,
        );
      }
      target[property] = value;
      return true;
...
let campState = createObservable(
  ...
  (prop, oldVal, newVal) => {
    ...
    if (prop === "food" && newVal < 10) {
      console.log(
        "[ALERT] rations critiques : convocation du conseil immédiate",
      );
    }
    if (prop === "security" && newVal === "low") {
      console.log(
        "[ALERT] niveau de sécurité critique : doublement des gardes",
      );
    }
  },
);
...
Un Proxy ajoute une couche d'indirection sur _chaque_ accès. Sur un objet accédé des millions de fois dans une boucle : ça se sent.
